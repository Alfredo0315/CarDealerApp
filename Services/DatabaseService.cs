...
namespace CarDealerApp.Services
{
    ...
    public class DatabaseService
    {
        ...
        {
            _connectionString =
                "Server=DESKTOP-4H4B3M6\\SQLEXPRESS;Database=Car_dealer;Trusted_Connection=True;TrustServerCertificate=True;";
        }
        ...
        public AuthResult? Authenticate(string email, string password)
        {
            try
            {
                ...
                var adminQuery = "SELECT ID_User, Role FROM Users WHERE Email=@e AND PasswordHash=@p AND IsActive=1";
                using (var cmd = new SqlCommand(adminQuery, conn))
                {
                    ...
                    if (r.Read())
                        return new AuthResult { Role = UserRole.Admin };
                }

               
                var clientQuery = "SELECT ID_Client FROM Client WHERE Email=@e AND PasswordHash=@p";
                using (var cmd = new SqlCommand(clientQuery, conn))
                {
                    ...
                    using var r = cmd.ExecuteReader();
                    if (r.Read())
                        return new AuthResult { Role = UserRole.Client, ClientId = r.GetInt32(0) };
                }

               
                var empQuery = "SELECT ID_Employee FROM Employee WHERE Email=@e AND PasswordHash=@p";
                using (var cmd = new SqlCommand(empQuery, conn))
                {
                    cmd.Parameters.AddWithValue("@e", email);
                    cmd.Parameters.AddWithValue("@p", password);
                    using var r = cmd.ExecuteReader();
                    if (r.Read())
                        return new AuthResult { Role = UserRole.Employee, EmployeeId = r.GetInt32(0) };
                }
        ...
        public bool RegisterClient(string name, string surname, string? middleName,
                                   ...
                                   out string error)
        {
            ...
            try
            {
                ...
                conn.Open();
                int count = 0;
                foreach (var table in new[] { "Client", "Employee", "Users" })
                {
                    using var chk = new SqlCommand(
                        $"SELECT COUNT(*) FROM {(table == "Users" ? "Users" : $"[{table}]")} WHERE Email=@e", conn);
                ...
                var insertSql = @"INSERT INTO Client
                    ([Name], Surname, Middle_name, Phone_number, Email, PasswordHash,
                     Passport_series, Passport_number)
                    VALUES (@name, @surname, @middle, @phone, @email, @pwd, 0, '0')";

                using var cmd = new SqlCommand(insertSql, conn);
                cmd.Parameters.AddWithValue("@name",    name);
                cmd.Parameters.AddWithValue("@surname", surname);
                cmd.Parameters.AddWithValue("@middle",  (object?)middleName ?? DBNull.Value);
        ...
        public int GetClientIdByLogin(string email)
        {
            try
            {
                using var conn = new SqlConnection(_connectionString);
                ...
                using var cmd = new SqlCommand(
                    "SELECT ID_Client FROM Client WHERE Email=@e", conn);
                cmd.Parameters.AddWithValue("@e", email);
        ...
        public void UpdateCar(Car car)
        {
            ...
            var query = @"UPDATE Car SET Mark=@mark, Model=@model, Color=@color,
                          ...
                          PhotoPath=@photo WHERE ID_Car=@id";
            using var cmd = new SqlCommand(query, conn);
        ...
        public void DeleteCar(int id)
        {
            ...
            using var cmd = new SqlCommand("DELETE FROM Car WHERE ID_Car=@id", conn);
        ...
        public List<Client> GetAllClients()
        {
            ...
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            var query = "SELECT ID_Client, [Name], Surname, Middle_name, Passport_series, Passport_number, Phone_number, Email FROM Client";
            using var cmd = new SqlCommand(query, conn);
            using var reader = cmd.ExecuteReader();
            while (reader.Read())
            {
        ...
        public void UpdateClient(Client client)
        {
            ...
            var query = @"UPDATE Client SET [Name]=@name, Surname=@surname, Middle_name=@middle,
                          ...
                          Phone_number=@phone, Email=@email WHERE ID_Client=@id";
            using var cmd = new SqlCommand(query, conn);
        ...
        public void DeleteClient(int id)
        {
            ...
            using var delCmd = new SqlCommand("DELETE FROM [Order] WHERE ID_Client=@id", conn);
            delCmd.Parameters.AddWithValue("@id", id);
            delCmd.ExecuteNonQuery();
            using var cmd = new SqlCommand("DELETE FROM Client WHERE ID_Client=@id", conn);
            cmd.Parameters.AddWithValue("@id", id);
            cmd.ExecuteNonQuery();
        }
        
        public bool ClientHasPassport(int clientId)
        {
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            using var cmd = new SqlCommand(
                "SELECT Passport_series, Passport_number FROM Client WHERE ID_Client=@id", conn);
            cmd.Parameters.AddWithValue("@id", clientId);
            using var r = cmd.ExecuteReader();
            if (!r.Read()) return false;
            var series = r.GetInt32(0);
            var number = r.GetValue(1)?.ToString();
            return series > 0 && !string.IsNullOrEmpty(number) && number != "0";
        }
        
        public void UpdateClientPassport(int clientId, int series, string number)
        {
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            using var cmd = new SqlCommand(
                "UPDATE Client SET Passport_series=@s, Passport_number=@n WHERE ID_Client=@id", conn);
        ...
        public List<Employee> GetAllEmployees()
        {
            ...
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            var query = "SELECT ID_Employee, [Name], Surname, Middle_name, Business_phone_number, Post, Email FROM Employee";
            using var cmd = new SqlCommand(query, conn);
            using var reader = cmd.ExecuteReader();
            while (reader.Read())
            {
        ...
        public void AddEmployee(Employee emp)
        {
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            var query = @"INSERT INTO Employee ([Name], Surname, Middle_name, Business_phone_number, Post, Email)
                          VALUES (@name, @surname, @middle, @phone, @post, @email)";
            using var cmd = new SqlCommand(query, conn);
        ...
        public void UpdateEmployee(Employee emp)
        {
            ...
            var query = @"UPDATE Employee SET [Name]=@name, Surname=@surname, Middle_name=@middle,
                          Business_phone_number=@phone, Post=@post, Email=@email WHERE ID_Employee=@id";
            using var cmd = new SqlCommand(query, conn);
        ...
        public void DeleteEmployee(int id)
        {
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            using var delCmd = new SqlCommand("DELETE FROM Employee_Car WHERE ID_Employee=@id", conn);
            delCmd.Parameters.AddWithValue("@id", id);
            delCmd.ExecuteNonQuery();
            using var cmd = new SqlCommand("DELETE FROM Employee WHERE ID_Employee=@id", conn);
            cmd.Parameters.AddWithValue("@id", id);
            cmd.ExecuteNonQuery();
        }
        ...
        public List<Diller> GetAllDillers()
        {
            ...
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            var query = "SELECT ID_Diller, Car_center_name, Phone_number, Email FROM Diller";
            using var cmd = new SqlCommand(query, conn);
            using var reader = cmd.ExecuteReader();
            while (reader.Read())
            {
        ...
        public void DeleteDiller(int id)
        {
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            using var delCmd = new SqlCommand("DELETE FROM Diller_Car WHERE ID_Diller=@id", conn);
            delCmd.Parameters.AddWithValue("@id", id);
            delCmd.ExecuteNonQuery();
            using var cmd = new SqlCommand("DELETE FROM Diller WHERE ID_Diller=@id", conn);
            cmd.Parameters.AddWithValue("@id", id);
            cmd.ExecuteNonQuery();
        }
        ...
        public void AddOrder(Order order, int carId)
        {
            ...
            using var linkCmd = new SqlCommand("INSERT INTO Order_Car (ID_Order, ID_Car) VALUES (@order, @car)", conn);
        ...
        public void UpdateOrder(Order order)
        {
            ...
            var query = @"UPDATE [Order] SET ID_Client=@client, Order_status=@status,
                          Date_of_execution=@date, Payment_method=@payment WHERE ID_Order=@id";
            using var cmd = new SqlCommand(query, conn);
        ...
        public void DeleteOrder(int id)
        {
            using var conn = new SqlConnection(_connectionString);
            conn.Open();
            using var delCmd = new SqlCommand("DELETE FROM Order_Car WHERE ID_Order=@id", conn);
            delCmd.Parameters.AddWithValue("@id", id);
            delCmd.ExecuteNonQuery();
            using var cmd = new SqlCommand("DELETE FROM [Order] WHERE ID_Order=@id", conn);
            cmd.Parameters.AddWithValue("@id", id);
            cmd.ExecuteNonQuery();
        }
        ...
        public string? GetEmployeePost(int employeeId)
        {
            ...
            try
            {
                ...
                conn.Open();
                using var cmd = new SqlCommand(
                    "SELECT Post FROM Employee WHERE ID_Employee=@id", conn);
        ...
        public int GetClientIdByPhone(string phone)
        {
            try
            {
                ...
                conn.Open();
                using var cmd = new SqlCommand(
                    "SELECT ID_Client FROM Client WHERE Phone_number=@p", conn);
        ...
        public List<(int Id, string Phone, string Name)> GetClientPhones()
        {
            ...
            try
            {
                ...
                conn.Open();
                using var cmd = new SqlCommand(
                    "SELECT ID_Client, Phone_number, Surname + ' ' + [Name] FROM Client ORDER BY Surname", conn);
                using var r = cmd.ExecuteReader();
                while (r.Read())
        ...
        public string GetClientPhoneById(int clientId)
        {
            try
            {
                ...
                conn.Open();
                using var cmd = new SqlCommand(
                    "SELECT Phone_number FROM Client WHERE ID_Client=@id", conn);
                cmd.Parameters.AddWithValue("@id", clientId);
                return cmd.ExecuteScalar()?.ToString() ?? "—";
            }
            catch { return "—"; }
